...
$$
\min_{\Theta} \sum_{r \in \mathcal{R}} ( \lambda \| C^{_}(r) - C(r, \Theta, t^{c}) \|_{2}^2 + \| C^{_}(r) - C(r, \Theta, \text{sort}(t^{c} \cup t^{f})) \|_{2}^2 )
$$
...
- $\phi_l(I_{\text{GT}})_{h,w}$ : The corresponding feature activation of the ground-truth image $ I_{\text{GT}} $ at the same location.  
...
The rays are rendred using the distances $t^{c}$ between the intervals $[t_{n}, t_{f}]$, $t^{c} \sim Dist{[t_n, t_f]} $, After the coarse MLP generates the vector of coarse $w^{c}$, fine distances $t^{f}$ are sampled from the histogram defined by $t^{c}$ and the $w^{c}$, using inverse transform sampling $t^{f} \sim Hist(t^{c}, w^{c})$.
